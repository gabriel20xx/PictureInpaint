...
# while in diffusers it split into scale, shift. Here we swap the linear projection weights in order to be able
# to use diffusers implementation
...
def convert_flux_transformer_checkpoint_to_diffusers(
    ...
    if has_guidance:
        converted_state_dict["time_text_embed.guidance_embedder.linear_1.weight"] = (
        ...
        converted_state_dict["time_text_embed.guidance_embedder.linear_2.weight"] = (
    ...
    for i in range(num_layers):
        ...
        converted_state_dict[f"{block_prefix}attn.norm_q.weight"] = (
            original_state_dict.pop(f"double_blocks.{i}.img_attn.norm.query_norm.scale")
        )
        converted_state_dict[f"{block_prefix}attn.norm_k.weight"] = (
            original_state_dict.pop(f"double_blocks.{i}.img_attn.norm.key_norm.scale")
        )
        converted_state_dict[f"{block_prefix}attn.norm_added_q.weight"] = (
            original_state_dict.pop(f"double_blocks.{i}.txt_attn.norm.query_norm.scale")
        )
        converted_state_dict[f"{block_prefix}attn.norm_added_k.weight"] = (
            original_state_dict.pop(f"double_blocks.{i}.txt_attn.norm.key_norm.scale")
        )
        # ff img_mlp
...
def main():
    original_ckpt = load_original_checkpoint()
    ...
    if transformer:
        ...
        converted_transformer_state_dict = (
        ...
        print(
            f"Saving Flux Transformer in Diffusers format. \
                Variant: {'guidance-distilled' if has_guidance else 'timestep-distilled'}"
        )
        transformer.to(dtype).save_pretrained(f"{output_path}/transformer")
    global vae
    if vae:
        ...
        vae = AutoencoderKL.from_config(
            config, scaling_factor=0.3611, shift_factor=0.1159
        ).to(torch.bfloat16)

        converted_vae_state_dict = convert_ldm_vae_checkpoint(original_ckpt, vae.config)
        vae.load_state_dict(converted_vae_state_dict, strict=True)
        vae.to(dtype).save_pretrained(f"{output_path}/vae")
